...
(function ($) {
    ...
    $modal.on('show.bs.modal', function (e) {
        ...
        if(template.length > 0) {
            ...
            // Adjust buttons based off mode
            var mode = target.data('mode');
            if(typeof(mode) != 'undefined' && mode) {
                if(mode == 'create') {
                    $form.find('[name="action"][value="delete"]')
                        .remove();
                } else if(mode == 'edit') {
    ...
    $modal.on('ajaxsubmit', function (event, data) {
        var $btn = data.btn;
        var redirect = $btn.data('successRedirect') ? $btn.data('successRedirect') : window.location;
        $form = form();
        $btns = btns();
                ...
                if(error.list) {
                    $form.find('input,textarea,select').each(function () {
                        ...
                        if(name in error.message) {
                            ...
                            $('<p class="help-block errormsg">' + error.message[name][0] + '</p>').insertAfter(
