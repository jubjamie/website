	...
	private function addEventToFinanceDb(Event $event)
	{
		if($event->type == Event::TYPE_EVENT) {
			$fields       = [
				...
				'data[Event][start_date]'  => Carbon::createFromFormat('d/m/Y', $event->start_date)->format('d-m-Y'),
				'data[Event][end_date]'    => Carbon::createFromFormat('d/m/Y', $event->end_date)->format('d-m-Y'),
